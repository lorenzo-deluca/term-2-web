...
| `127.0.0.1:16666` | **Raw TCP socket** | Connect via `nc`, `telnet`, or custom scripts. Override with `SER2NET_HOST_PORT`. |
...
4. **Monitoring**: The **Watchdog** tracks the trace file size. If no bytes are written for 900 seconds, it issues a Docker API restart command to `ser2net` to clear stuck buffers.
...
|---|---|---|
| `SILENCE_THRESHOLD_SEC` | `900` | Seconds of no data before triggering a pipeline restart. **Increase this if your device legitimately stays silent for >15 minutes.** |
| `CHECK_INTERVAL_SEC` | `30` | Trace file polling interval. |
